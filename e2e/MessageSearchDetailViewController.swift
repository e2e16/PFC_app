...
class MessageSearchDetailViewController: UIViewController {
  ...
  func savedButtonAction() {
    ...
    if message?.saved! == false {
      myTempList = MessageList()
      myTempList.addToSavedList(message!)
      toggleSavedButtonIcon(true)
    } else {
      myTempList = MessageList()
      myTempList.deleteFromSavedList(messIndex!)
